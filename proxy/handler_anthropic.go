...
func (h *Handler) Messages(c *gin.Context) {
	...
	effectiveModel := strings.TrimSpace(gjson.GetBytes(codexBody, "model").String())
	if effectiveModel == "" {
		effectiveModel = model
	}
	if strings.HasPrefix(strings.ToLower(effectiveModel), "gpt-image-") {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if account == nil {
			...
			if account == nil {
				...
				sendAnthropicError(c, http.StatusServiceUnavailable, "overloaded_error", fmt.Sprintf("No available account for model %s", effectiveModel))
				return
			}
		}
